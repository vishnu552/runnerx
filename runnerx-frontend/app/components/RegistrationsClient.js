...
export default function RegistrationsClient({ registrations: initialRegistrations }) {
  ...
  const openModal = (type, item, reg) => {
    ...
    const siblings = reg.lineItems.filter(li =>
      (li.participantName || '').toLowerCase().trim() === (item.participantName || '').toLowerCase().trim() &&
      (li.participantEmail || '').toLowerCase().trim() === (item.participantEmail || '').toLowerCase().trim() &&
      (li.participantPhone || '').trim() === (item.participantPhone || '').trim()
    );
      ...
      const results = await Promise.all(
        selectedSiblings.map(li => 
          authenticatedFetch(`/api/auth/registrations/line-item/${li.id}`, {
            method: 'PATCH',
            body: JSON.stringify({ tshirtSize: formData.tshirtSize, isProfileUpdated: true }),
          }).then(r => r.json())
        )
      );
      if (results.every(r => r.success)) {
        setMessage({ type: 'success', text: 'T-Shirt size updated successfully! Refreshing...' });
  ...
  const checkBibAvailability = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/bibs/check?eventId=${selectedReg.eventId}&bib=${bib}`);
      ...
      const options = {
        ...
        handler: async function (response) {
          ...
          if (verifyData.success) {
            setMessage({ type: 'success', text: 'VIP Bib purchased successfully! Refreshing...' });
            setTimeout(() => window.location.reload(), 1500);
          } else {
    ...
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        {registrations.map((reg) => {
          const now = new Date();
          const regEnd = reg.event?.registrationEnd ? new Date(reg.event.registrationEnd) : null;
              ...
              {(() => {
                ...
                return grouped.map((group) => {
                  ...
                  const sorted = [...group.items].sort((a, b) => {
                    const aVirtual = (a.raceTypeSnapshot || a.distanceSnapshot || '').toLowerCase().includes('virtual') ? 1 : 0;
                    const bVirtual = (b.raceTypeSnapshot || b.distanceSnapshot || '').toLowerCase().includes('virtual') ? 1 : 0;
                    return aVirtual - bVirtual;
                  });
                  const primary = sorted[0];
                  const isRegistrant = sorted.some(li => li.isRegistrant);
                  const isProfileUpdated = sorted.some(li => li.isProfileUpdated);

                  return (
                    <div key={group.key} className="participant-row">
                      <div className="participant-top" style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1, minWidth: 0 }}>
                        ...
                        <div className="participant-info">
                          ...
                          <div className="participant-chips">
                            ...
                            {sorted.map((li) => li.bibNumber ? (
                              <span key={`bib-${li.id}`} className="chip-bib">BIB #{li.bibNumber}</span>
                            ) : null)}
                          </div>
                        </div>
                      </div>
      ...
      {activeTab && (
        <div style={{
          ...
          <div style={{
            background: 'var(--card-bg, #fff)', width: '100%', maxWidth: '620px',
            ...
            <div style={{
              ...
              background: activeTab === 'edit' ? 'linear-gradient(90deg,#ffc83c22,transparent)' : undefined,
            }}>
              <div>
                <h2 style={{ fontSize: '1.15rem', fontWeight: 800, textTransform: 'uppercase', fontStyle: 'italic', fontFamily: 'var(--font-heading, sans-serif)', margin: 0 }}>
                  {activeTab === 'view' ? '👁 Participant Details' : '✎ Change & Upgrade Entry'}
                </h2>
                {selectedItem && (
                  <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', margin: '2px 0 0' }}>
                    {selectedItem.participantName} · {selectedReg?.eventTitleSnapshot}
                  </p>
                )}
              </div>
              <button onClick={closeModal} style={{ background: 'none', border: 'none', fontSize: '1.6rem', cursor: 'pointer', color: 'var(--text-muted)', lineHeight: 1 }}>×</button>
            </div>
            ...
            <div style={{ padding: '24px', overflowY: 'auto' }}>
              {activeTab === 'view' ? (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                  ...
                  <DetailItem label="T-Shirt Size" value={selectedItem.tshirtSize || selectedSiblings.find(li => li.tshirtSize)?.tshirtSize} />
                  <div style={{ gridColumn: 'span 2' }}>
                    <DetailItem label={selectedSiblings.length > 1 ? 'Categories' : 'Category'} value={selectedSiblings.map(li => `${li.categoryNameSnapshot} · ${li.distanceSnapshot || li.raceTypeSnapshot}`).join('  |  ')} />
                  </div>
                  {selectedSiblings.filter(li => li.bibNumber).map(li => (
                    <DetailItem key={li.id} label={`BIB (${li.categoryNameSnapshot})`} value={`#${li.bibNumber}`} highlight />
                  ))}
                  <div style={{ gridColumn: 'span 2' }}>
                    <DetailItem label="Address" value={[selectedItem.participantAddress, selectedItem.participantCity, selectedItem.participantState, selectedItem.participantPinCode].filter(Boolean).join(', ') || '—'} />
                  </div>
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  {message && (
                    <div style={{
                      padding: '12px 14px', borderRadius: '10px', fontSize: '0.88rem',
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: '0.73rem', fontWeight: 700, textTransform: 'uppercase', marginBottom: '8px', color: 'var(--text-muted)' }}>What do you want to change?</label>
                    <select value={updateType} onChange={(e) => { setUpdateType(e.target.value); setBibCheckStatus(null); }} className="form-input w-full" style={{ padding: '12px', fontSize: '1rem', fontWeight: 600 }}>
                  ...
                  {updateType === 'BIB' && (
                    <div style={{ background: 'var(--surface-alt)', padding: '20px', borderRadius: '12px', border: '1px solid var(--border)' }}>
                      <div style={{ marginBottom: '20px', padding: '12px', background: 'rgba(99,102,241,0.1)', borderRadius: '8px', border: '1px solid rgba(99,102,241,0.2)' }}>
                        <p style={{ fontSize: '0.8rem', color: '#6366f1', margin: 0, fontWeight: 700, textTransform: 'uppercase' }}>Currently Allocated Bib</p>
                        <p style={{ fontSize: '1.4rem', fontWeight: 900, color: '#4f46e5', margin: '4px 0 0' }}>{selectedItem.bibNumber ? `#${selectedItem.bibNumber}` : 'Not Allocated Yet'}</p>
                      </div>

                      <label style={{ display: 'block', fontSize: '0.73rem', fontWeight: 700, textTransform: 'uppercase', marginBottom: '8px', color: 'var(--text-muted)' }}>Enter Desired VIP Bib Number</label>
                      <div style={{ display: 'flex', gap: '10px' }}>
                        <input
                          type="text"
                          name="bibNumber"
                          value={formData.bibNumber}
                          onChange={(e) => { handleInputChange(e); setBibCheckStatus(null); }}
                          placeholder="e.g. 9999"
                          className="form-input"
                          style={{ flex: 1, padding: '12px', fontSize: '1.2rem', fontWeight: 700, letterSpacing: '0.05em' }}
                        />
                        <button 
                          onClick={checkBibAvailability}
                          disabled={!formData.bibNumber?.trim() || bibCheckStatus === 'checking'}
                          style={{ padding: '0 20px', background: '#1a1a2e', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 700, cursor: 'pointer' }}
                        >
                          Check
                        </button>
                      </div>

                      {bibCheckStatus === 'checking' && <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '10px', fontWeight: 600 }}>Checking availability...</p>}
                      {bibCheckStatus === 'available' && <p style={{ fontSize: '0.9rem', color: '#16a34a', marginTop: '10px', fontWeight: 700 }}>✓ VIP Bib #{formData.bibNumber} is available!</p>}
                      {bibCheckStatus === 'unavailable' && <p style={{ fontSize: '0.9rem', color: '#dc2626', marginTop: '10px', fontWeight: 700 }}>✗ Bib #{formData.bibNumber} is already taken. Try another.</p>}
                      {bibCheckStatus === 'error' && <p style={{ fontSize: '0.9rem', color: '#dc2626', marginTop: '10px', fontWeight: 700 }}>Failed to check availability.</p>}

                      <div style={{ marginTop: '24px', paddingTop: '16px', borderTop: '1px dashed var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                          <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', margin: 0, fontWeight: 600, textTransform: 'uppercase' }}>VIP Bib Price</p>
                          <p style={{ fontSize: '1.4rem', fontWeight: 900, color: 'var(--text)', margin: 0 }}>₹100</p>
                        </div>
                      </div>
                    </div>
                  )}
            ...
            <div style={{ padding: '18px 24px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
              <button onClick={closeModal} style={{ padding: '10px 20px', borderRadius: '8px', border: '1px solid var(--border)', background: 'transparent', fontWeight: 600, cursor: 'pointer', color: 'var(--text)' }}>
                Close
              </button>
              {activeTab === 'edit' && updateType === 'TSHIRT' && (
                <button
                  ...
                  style={{
                    ...
                    background: '#ffc83c', color: '#000', fontWeight: 700,
                    cursor: saving || !formData.tshirtSize ? 'not-allowed' : 'pointer', opacity: saving || !formData.tshirtSize ? 0.7 : 1,
                  }}
                >
              ...
              {activeTab === 'edit' && updateType === 'BIB' && (
                <button
                  ...
                  style={{
                    padding: '10px 24px', borderRadius: '8px', border: 'none',
                    background: bibCheckStatus === 'available' ? '#16a34a' : 'var(--surface-alt)', 
                    color: bibCheckStatus === 'available' ? '#fff' : 'var(--text-muted)', 
                    fontWeight: 700,
                    cursor: bibCheckStatus === 'available' && !bibPaying ? 'pointer' : 'not-allowed', 
                  }}
                >
...
function DetailItem({ label, value, highlight }) {
  return (
    <div>
      <p style={{ fontSize: '0.68rem', textTransform: 'uppercase', color: 'var(--text-muted)', fontWeight: 700, marginBottom: '4px' }}>{label}</p>
      <p style={{ fontSize: '0.95rem', fontWeight: 600, color: highlight ? '#6366f1' : 'var(--text)', margin: 0 }}>{value || '—'}</p>
    </div>
  );
}
...
function InputGroup({ label, name, value, onChange, required = false, type = 'text' }) {
  return (
    <div>
      <label style={{ display: 'block', fontSize: '0.73rem', fontWeight: 700, textTransform: 'uppercase', marginBottom: '6px', color: 'var(--text-muted)' }}>{label}</label>
      <input
        type={type}
        name={name}
        value={value}
