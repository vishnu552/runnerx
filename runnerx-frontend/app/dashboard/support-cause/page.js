  ...
  if (step === 3 && success) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', animation: 'fadeIn 0.5s ease' }}>
        <div className="card" style={{ padding: '64px 32px', textAlign: 'center', background: 'white' }}>
          <div style={{
          ...
          <h2 style={{ fontSize: '2.2rem', fontWeight: 900, color: 'var(--text)', marginBottom: '16px', textTransform: 'uppercase', fontStyle: 'italic' }}>
            Heroic Contribution!
          </h2>
          <p style={{ color: 'var(--text-muted)', maxWidth: 520, margin: '0 auto 32px', fontSize: '1.1rem', lineHeight: 1.6 }}>
            Your donation of <strong style={{ color: 'var(--primary)', fontSize: '1.2rem' }}>₹{Number(formData.amount).toLocaleString('en-IN')}</strong> to <span style={{ fontWeight: 700 }}>{formData.causeName}</span> has been received successfully. Every step you run and every rupee you give makes the world a better place.
          </p>
          <div style={{ display: 'flex', gap: '16px', justifyContent: 'center' }}>
            <button
              ...
              style={{ padding: '16px 32px', fontSize: '1rem', fontStyle: 'italic' }}
            >
              Support Another Cause
            </button>
            <a href="/dashboard/orders" className="btn btn-outline" style={{ padding: '16px 32px', fontSize: '1rem', fontStyle: 'italic', borderRadius: '12px' }}>
              View My Orders →
            </a>
          </div>
        </div>
                ...
                {amountOptions.map(amt => (
                    <button
                      ...
                      style={{
                        ...
                        background: formData.amount === String(amt) ? '#ffc83c' : 'var(--surface-alt)',
                        color: formData.amount === String(amt) ? '#000' : 'var(--text)',
                        border: formData.amount === String(amt) ? '2px solid #ffc83c' : '1px solid var(--border)',
                        boxShadow: formData.amount === String(amt) ? '0 4px 15px rgba(255,200,60,0.3)' : 'none',
                        transition: 'all 0.2s cubic-bezier(0.4, 0, 0.2, 1)',
                        transform: formData.amount === String(amt) ? 'translateY(-2px)' : 'translateY(0)',
                      }}
                    >
