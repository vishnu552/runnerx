...
function PolicyContent() {
  ...
  useEffect(() => {
    async function fetchPolicies() {
      try {
        const types = ['PRIVACY', 'TERMS', 'REFUND', 'WAIVER'];
        const results = await Promise.all(
          types.map(type => getInfoSections(type, 'GLOBAL'))
        );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {policyGroups.map((group) => {
          const isOpen = openType === group.id;
          const groupPolicies = policies[group.id] || [];
              ...
              {isOpen && (
                <div style={{
                  ...
                  {groupPolicies.length === 0 ? (
                    <p style={{ color: 'var(--text-muted)', paddingTop: '20px', textAlign: 'center' }}>
                      No sections available for this policy yet.
                    </p>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', paddingTop: '24px' }}>
                      {groupPolicies.map((section, idx) => (
