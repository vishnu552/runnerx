...
import { getUserRegistrations, getActiveEvent } from '@/app/lib/api';
import { getOriginFromCookies } from '@/app/lib/origin';
import RegistrationsClient from '@/app/components/RegistrationsClient';
...
export default async function MyRegistrationsPage() {
  ...
  const { code: originCode } = await getOriginFromCookies();
  const activeEvent = originCode ? await getActiveEvent(originCode) : null;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', minHeight: '600px' }}>
      ...
      {registrations.length > 0 ? (
        ...
        <div className="card" style={{ padding: '64px 24px', textAlign: 'center', borderStyle: 'dashed', borderWidth: '2px' }}>
          ...
          <a href="/dashboard/register" className="btn btn-primary" style={{ padding: '14px 32px', fontStyle: 'italic', fontSize: '1rem' }}>
            Browse Events →
          </a>
        </div>
      )}
